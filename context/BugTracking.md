...
## [2025-08-25] – Email modal: mailto recipients fix & notification sender name
**Feature/Bug:** Mailto link generated by `EmailModal` didn't prefill recipients correctly (was using query param `to=`). Notifications created after sending included the sender id; UI should show sender name in notification messages.
**Fix:** Updated `generateMailtoLink` in `src/components/EmailModal.tsx` to use `mailto:recipient1,recipient2?subject=...&body=...`. Ensured notification message uses `${currentUser?.name}` when creating notifications in `EmailModal`.
**Files changed:** `src/components/EmailModal.tsx`, `src/components/SettingsView.tsx` (UI additions for notificationPreferences)
**Verify:** Open Email modal → select recipients → choose Browser Email Client → click Open Email Client → email app opens with recipients prefilled. Check notifications table/UI shows message with sender name (e.g., "Alice sent you a report: Monthly Budget").
**Status:** ✅ Completed
